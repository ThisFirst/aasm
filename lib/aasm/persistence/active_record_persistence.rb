      ...
      module ClassMethods
        def aasm_create_scope(state_machine_name, scope_name, state_name)
          if ActiveRecord::VERSION::MAJOR >= 3
            ...
            class_eval { scope scope_name, lambda { where(table_name => conditions) } }
          else
            conditions = { table_name => { aasm(state_machine_name).attribute_name => state_name.to_s } }
            class_eval { named_scope scope_name, conditions: conditions }
          end
        end
      end
      ...
      module InstanceMethods
        ...
        def aasm_execute_after_commit
          begin
            ...
            unless Gem::Version.new(::AfterCommitEverywhere::VERSION) >= Gem::Version.new('0.1.5')
              raise LoadError
            end
  ...
  [DEPRECATION] :after_commit AASM callback is not safe in terms of race conditions and redundant calls.
        ...
        def aasm_update_column(attribute_name, value)
          self
            ...
            .update_all(attribute_name => value) == 1
        end
        ...
        def aasm_transaction(requires_new, requires_lock)
          self
            .class
            .transaction(requires_new: requires_new) do
        ...
        def aasm_enum(name = :default)
          ...
          when false
            nil
          when true
            aasm_guess_enum_method(name)
          when nil
            aasm_guess_enum_method(name) if aasm_column_looks_like_enum(name)
          else
            AASM::StateMachineStore.fetch(self.class, true).machine(name).config.enum
          end
        end
        ...
        def aasm_ensure_initial_state
          AASM::StateMachineStore
            ...
            .each do |state_machine_name|
              ...
              aasm(state_machine_name).enter_initial_state if aasm_column_is_blank?(state_machine_name)
            end
        end
        ...
        def aasm_column_is_blank?(state_machine_name)
          ...
          attribute_names.include?(attribute_name.to_s) &&
            (
              if send(attribute_name).respond_to?(:empty?)
                !!send(attribute_name).empty?
              else
                !send(attribute_name)
              end
            )
        ...
        def aasm_invalid_state?(state_machine_name)
          aasm(state_machine_name).current_state &&
            !aasm(state_machine_name).states.include?(aasm(state_machine_name).current_state)
        end
      end # InstanceMethods
    end
  end # Persistence
